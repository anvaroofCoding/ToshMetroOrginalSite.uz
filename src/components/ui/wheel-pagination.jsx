...
export default function WheelPagination({
  ...
  return (
    <div
      ref={containerRef}
      className={cn("flex items-center gap-2 p-4 select-none cursor-pointer", className)}>
      {/* Previous arrow */}
      <Button
        variant="ghost"
        size="icon"
        onClick={prevPage}
        disabled={active === 0}
        className="text-blue-400 hover:text-blue-900 disabled:opacity-40 transition-colors">
        ...
        {visiblePages.map((p) => (
          <motion.div
            ...
            className={cn(
              ...
              active === p
                ? "bg-blue-900 text-white border border-blue-900 shadow-md"
