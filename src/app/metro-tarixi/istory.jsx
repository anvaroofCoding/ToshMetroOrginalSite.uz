...
export default function TashkentMetroPage() {
	...
	const benefits = [
		{
			icon: Users,
			title: "Kunlik yo'lovchilar",
			value: '500,000+',
			description:
				"Har kuni yuz minglab yo'lovchilarga ishonchli xizmat ko'rsatamiz",
		},
		{
			icon: Leaf,
			title: 'Ekologik toza',
			value: 'Nol emissiya',
			description:
				'Elektr poyezdlari shahar ifloslanishini va uglerod izini kamaytiradi',
		},
		{
			icon: Shield,
			title: 'Xavfsizlik rekordi',
			value: '99.9%',
			description:
				"Zamonaviy xavfsizlik tizimlari bilan ajoyib xavfsizlik ko'rsatkichi",
		},
		{
			icon: Clock,
			...
			value: '98%',
			description:
				"Poyezdlar jadval bo'yicha minimal kechikishlar bilan harakat qiladi",
		},
	]
	...
	const stats = [
		{
			label: 'Umumiy uzunlik',
			value: animatedStats[0],
			unit: 'km+',
			icon: MapPin,
		},
		{ label: 'Bekatlar', value: animatedStats[1], unit: '', icon: Train },
		{ label: "Yo'nalishlar", value: animatedStats[2], unit: '', icon: Zap },
		{
			label: 'Xizmat yillari',
			...
			<div className='container mx-auto px-4 py-8 max-w-7xl relative'>
				...
				<div ref={el => (sectionRefs.current[2] = el)} className='mb-20'>
					<div className='grid grid-cols-2 md:grid-cols-4 gap-6'>
						{stats.map((stat, index) => (
							<Card
								key={index}
								className='text-center border-0 shadow-xl hover:shadow-2xl transition-all duration-500 hover:-translate-y-2 bg-gradient-to-br from-white to-blue-50 group'
								style={{
				...
				<div ref={el => (sectionRefs.current[3] = el)} className='mb-20'>
					...
					<p className='text-center text-gray-600 mb-12 max-w-2xl mx-auto'>
						Butun metropolitan hududiga zamonaviy, samarali va xavfsiz transport
						bilan xizmat ko&apos;rsatadigan to&apos;rtta o&apos;zaro
						bog&apos;langan yo&apos;nalish
					</p>
				...
				{/* Benefits Section */}
				<div ref={el => (sectionRefs.current[4] = el)} className='mb-20'>
					<h2 className='text-4xl font-bold text-[#0E327F] mb-4 text-center'>
						Nima uchun Metroni Tanlash Kerak
					</h2>
					<p className='text-center text-gray-600 mb-12 max-w-2xl mx-auto'>
						Zamonaviy, barqaror shahar transportining afzalliklarini his eting
					</p>
					<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
						{benefits.map((benefit, index) => (
							<Card
								key={index}
								className='text-center border-0 shadow-lg hover:shadow-xl transition-all duration-500 hover:-translate-y-3 bg-gradient-to-br from-white to-blue-50 group'
								style={{
							...
							>
								<CardContent className='pt-8 pb-6'>
									<div className='inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-r from-[#0E327F] to-blue-600 mb-6 group-hover:rotate-12 group-hover:scale-110 transition-all duration-300'>
										<benefit.icon className='w-8 h-8 text-white' />
									</div>
									<div className='text-3xl font-bold text-[#0E327F] mb-2 group-hover:scale-110 transition-transform duration-300'>
										{benefit.value}
									</div>
							...
							>
								<CardContent className='p-8'>
									...
									<div className='flex items-center justify-between'>
										...
										<ArrowRight className='w-5 h-5 text-[#0E327F] group-hover:translate-x-2 transition-transform duration-300' />
									</div>
								</CardContent>
							</Card>
						))}
				...
				<Card className='border-0 shadow-2xl bg-gradient-to-r from-[#0E327F] via-blue-600 to-blue-700 text-white overflow-hidden relative'>
					...
					<CardContent className='p-12 text-center relative z-10'>
						...
						<p className='text-xl mb-8 opacity-90 leading-relaxed max-w-4xl mx-auto'>
							...
							zamonaviy bekat bilan faoliyat yuritmoqda. Biz jamoamizga xavfsiz,
						...
						<div className='flex flex-wrap justify-center gap-4'>
							<Badge
								variant='secondary'
								className='bg-white/20 text-white text-lg px-6 py-3 hover:bg-white/30 transition-colors duration-300'
							>
								🏆 Markaziy Osiyoning Kashshofi
							</Badge>
							<Badge
								variant='secondary'
								className='bg-white/20 text-white text-lg px-6 py-3 hover:bg-white/30 transition-colors duration-300'
							>
								🌟 5 O&apos;n Yillik Mukammallik
							</Badge>
							<Badge
								variant='secondary'
								className='bg-white/20 text-white text-lg px-6 py-3 hover:bg-white/30 transition-colors duration-300'
