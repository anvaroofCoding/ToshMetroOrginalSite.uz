...
import { ChevronDown, CreditCard, GraduationCap, Heart, QrCode, Smartphone, Users, Wallet } from "lucide-react"
	...
	const transportCards = [
		{
			...
			requirements: "Sotib olish va to'lash uchun hujjatlar: pasport va talaba ID nusxasi",
			benefits: [
	...
	const paymentMethods = [
		{
			...
			features: ["Kontaktsiz to'lov", "Metro va avtobusda", "UZCARD qo'llab-quvvatlash"],
		},
		{
			id: "qr",
			name: "QR-bilet",
		...
		<div className="min-h-screen bg-white">
			<div className="container mx-auto px-4 py-6 sm:py-8 space-y-6 sm:space-y-8">
				...
				<Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
					<TabsList className="grid h-auto w-full grid-cols-2 rounded-lg p-1 ">
						{[
							{
								value: "cards",
								icon: CreditCard,
							...
							<TabsTrigger
								key={tab.value}
								value={tab.value}
								className="relative rounded-md px-3 py-2 sm:px-4 sm:py-3  text-sm font-medium transition-all duration-300 text-gray-600 data-[state=active]:text-white data-[state=active]:bg-blue-900 hover:text-gray-800 data-[state=active]:hover:text-white   
              ">
								<span className="flex items-center gap-2">
									<tab.icon className="h-4 w-4" />
									<span className="hidden sm:inline">{tab.label}</span>
									<span className="sm:hidden">{tab.value === "cards" ? "Kartalar" : "To'lovlar"}</span>
								</span>
							</TabsTrigger>
						))}
					</TabsList>
					<TabsContent value="cards" className="space-y-6 mt-6 sm:mt-8">
						...
						>
							{transportCards.map((card, index) => (
								...
								>
									<Card
										className={`cursor-pointer transition-all duration-500 hover:shadow-lg bg-white border border-gray-200 overflow-hidden group ${selectedCard === card.id ? "ring-2 ring-blue-500 shadow-lg" : ""
											}`}
									...
									>
										...
										<CardContent className="space-y-4 p-4 sm:p-6 pt-0">
											...
											<motion.div
												...
												whileHover={{ x: 5 }}
											>
												<span className="text-sm font-medium">Batafsil ma'lumot</span>
												<motion.div
													animate={{
												...
												{selectedCard === card.id && (
													...
													>
														<div className="bg-blue-50 p-3 sm:p-4 rounded-lg">
															<h4 className="font-semibold text-sm mb-2 text-blue-900">Hujjatlar:</h4>
															<p className="text-sm text-gray-700 leading-relaxed">{card.requirements}</p>
														</div>
														{card.pricing && (
															<div className="bg-green-50 p-3 sm:p-4 rounded-lg">
																<h4 className="font-semibold text-sm mb-2 text-green-900">Narx tarkibi:</h4>
														...
														{card.benefits && (
															<div className="bg-purple-50 p-3 sm:p-4 rounded-lg">
																<h4 className="font-semibold text-sm mb-2 text-purple-900">Imtiyozlar:</h4>
																<ul className="text-sm text-gray-700 space-y-1">
								...
								>
									<Card className="hover:shadow-lg transition-all duration-500 bg-white border border-gray-200 group overflow-hidden relative">
										<CardHeader className="text-center relative z-10 p-4 sm:p-6">
											...
											<CardDescription className="text-sm text-gray-600">{method.description}</CardDescription>
										</CardHeader>
				...
				>
					<Card className="bg-white border border-gray-200">
						<CardHeader className="text-center p-4 sm:p-6">
							<CardTitle className="text-xl sm:text-2xl font-bold text-blue-900">Tezkor Amallar</CardTitle>
							<CardDescription className="text-base sm:text-lg">Eng ko'p ishlatiladigan funksiyalar</CardDescription>
						</CardHeader>
