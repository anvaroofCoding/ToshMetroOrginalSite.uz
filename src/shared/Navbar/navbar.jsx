...
const menuItems = [
  ...
  },
  {
    ...
    dropdown: true,
    icon: UserCheck,
    dropdownItems: [{ label: "Umumiy ma'lumot", href: "umumiy-malumot", icon: Info }],
  },
  {
    label: "Bog'lanish",
    href: "",
        ...
        >
          ...
          >
            <motion.div className="flex items-center gap-2" whileHover={{ scale: 1.02 }} transition={{ duration: 0.2 }}>
              ...
              >
                O'zbekiston Respublikasi Transport vazirligi <span>"Toshkent Metropoliteni"</span> DUK
              </motion.h1>
            </motion.div>
                ...
                >
                  ...
                  {index < menuItems.length - 1 && (
                    <motion.span
                      ...
                      animate={{ opacity: hoveredItem === index || hoveredItem === index + 1 ? 0.3 : 0.6 }}
                      transition={{ duration: 0.2 }}
                    >
                      |
                    </motion.span>
                      ...
                      >
                        <div className="bg-[#0E327F]/95 backdrop-blur-xl rounded-2xl  border border-white/20 overflow-hidden">
                          {item.dropdownItems.map((subItem, subIndex) => (
                            <motion.div key={subItem.label} variants={itemVariants} custom={subIndex}>
                              <Link
                                href={subItem.href}
            ...
            <div className="flex items-center gap-4">
              <div className="hidden 2xl:flex items-center relative dropdown-container">
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                >
                  <motion.div animate={{ rotate: isLangOpen ? 360 : 0 }} transition={{ duration: 0.3 }}>
                    <Globe size={20} />
                  </motion.div>
                  {currentLang}
              ...
              <div className="flex items-center justify-between h-[90px]">
                ...
                >
                  ...
                  <h2 className="text-white text-[10px] capitalize font-medium">
                    ...
                    <span className="text-blue-200 font-semibold">"Toshkent Metropoliteni"</span> DUK
                  </h2>
                </motion.div>
                <motion.button
                  onClick={toggleMenu}
    ...
    >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          {item.dropdown ? (
            <span className="hover:opacity-80 transition-opacity font-medium">{item.label}</span>
          ) : (
            <Link href={item.href} className="hover:opacity-80 transition-opacity font-medium">
              {item.label}
            </Link>
          )}
        </div>
